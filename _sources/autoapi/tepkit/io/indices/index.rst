tepkit.io.indices
=================

.. py:module:: tepkit.io.indices


Attributes
----------

.. autoapisummary::

   tepkit.io.indices.EMPTY_INDEX
   tepkit.io.indices.VECTOR2D_INDICES
   tepkit.io.indices.VECTOR3D_INDICES
   tepkit.io.indices.TENSOR2D_ORDER2_INDICES
   tepkit.io.indices.TENSOR3D_ORDER2_INDICES
   tepkit.io.indices.VOIGT_NOTATION_2X2
   tepkit.io.indices.VOIGT_NOTATION_3X3
   tepkit.io.indices.V3D_INDICES
   tepkit.io.indices.T3D_INDICES


Module Contents
---------------

.. py:data:: EMPTY_INDEX
   :no-index:
   :value: ['-']


.. py:data:: VECTOR2D_INDICES
   :no-index:
   :value: ['x', 'y']


.. py:data:: VECTOR3D_INDICES
   :no-index:
   :value: ['x', 'y', 'z']


.. py:data:: TENSOR2D_ORDER2_INDICES
   :no-index:
   :value: ['xx', 'xy', 'yx', 'yy']


.. py:data:: TENSOR3D_ORDER2_INDICES
   :no-index:
   :value: ['xx', 'xy', 'xz', 'yx', 'yy', 'yz', 'zx', 'zy', 'zz']


.. py:data:: VOIGT_NOTATION_2X2
   :no-index:
   :value: ['xx', 'yy', 'xy']


.. py:data:: VOIGT_NOTATION_3X3
   :no-index:
   :value: ['xx', 'yy', 'zz', 'yz', 'xz', 'xy']


.. py:data:: V3D_INDICES
   :no-index:


.. py:data:: T3D_INDICES
   :no-index:


